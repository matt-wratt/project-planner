...
const levels = Array.apply(Array, Array(5)).map((_, i) => 1 / 4 * i).reverse()

const levelIndex = level => levels.reduce((x, y, i) => y >= level ? i : x, 0)
  ...
  render() {
    ...
    return (
      <div className="battery">
        { bars.map((status, i) => <div key={ i } className={ `battery-bar ${status}` } />) }
      </div>
    )
  }
