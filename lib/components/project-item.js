...
import { Alert, Button, Glyphicon, ModalTrigger, Nav, NavItem } from 'react-bootstrap'
...
export default React.createClass({
  ...
  render() {
    ...
    var { id, title, type, comments } = item
    ...
    type = type || 'story'
    return (
      <Alert id={ id } bsStyle={ this.alertStyle(item) } style={{ opacity }} { ...this.dragSourceFor('story') } { ...this.dropTargetFor('story') }>
        ...
        <span className={ `label label-${typeStyle(type)}` }>{ type }</span>
        {' '}{ title }
